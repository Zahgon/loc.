...
/** No pattern in the file applied to the path. @type {MatchResult} */
export const MATCH_NONE = "none";
/** The last pattern that matched was an ignore rule. @type {MatchResult} */
export const MATCH_IGNORE = "ignore";
/** The last pattern that matched was a whitelist (`!`) rule. @type {MatchResult} */
export const MATCH_WHITELIST = "whitelist";
...
export class Gitignore {
  ...
  matched(absPath, isDir) {
    if (this.globs.length === 0) return MATCH_NONE;
    ...
    if (candidate.length === 0) return MATCH_NONE;
    ...
    for (let i = this.globs.length - 1; i >= 0; i -= 1) {
      ...
      if (glob.regex.test(candidate)) {
        return glob.isWhitelist ? MATCH_WHITELIST : MATCH_IGNORE;
      }
    }
    return MATCH_NONE;
  }
