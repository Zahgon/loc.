...
/**
 * Returns null when no oracle is reachable: LOC_RUST_BIN is unset, the Rust
 * checkout is absent, or cargo is not installed. The caller turns that into a
 * skip. Plain `node --test` discovery sweeps this directory even though the
 * package's own test script globs only `test/**\/*.test.js`, so throwing here
 * would report a missing developer prerequisite as a failing test.
 *
 * @returns {string | null}
 */
function resolveRustBinary() {
  ...
  if (!existsSync(RUST_REPO)) return null;
...
if (RUST_BIN === null) {
  process.stderr.write(
    "skipping the differential harness: no Rust oracle available. " +
      "Set LOC_RUST_BIN to a prebuilt binary, or install cargo next to the " +
      `Rust checkout at ${RUST_REPO}.\n`,
  );
  process.exit(0);
}
