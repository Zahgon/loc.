## 0.5.0 (JavaScript port)

`loc-js` is a port of the Rust `loc` v0.5.0 to Node.js. It aims for exact
behavioural equivalence rather than improvement; see DIFFERENCES.md for the
short list of places where the two cannot agree, and for the bugs that are
reproduced on purpose.

- Ported `count`, `lang_from_ext`, `counter_config_for_lang`, `check_shebang`,
  the clap v2 command line, the `ignore`-crate directory walker and the table
  renderer.
- The `Lang` enum, extension table, comment configuration and shebang table are
  generated from the Rust source by `tools/gen-tables.mjs`; `npm run
  check:tables` fails on drift.
- Added a differential harness (`npm run test:diff`) that compares stdout,
  stderr and exit codes against the Rust binary over a corpus of repositories
  and a 25-entry argument matrix, plus a generator for a synthetic torture tree.
- Output is now deterministic. The Rust original inherits `HashMap` iteration
  randomness and thread scheduling, so rows with equal sort keys — and, in
  `--files` mode, whole language blocks — come out in an arbitrary order.
- Threading uses `worker_threads` with batched work rather than a per-file
  work-stealing deque. `LOC_JS_THREADS=1` forces the single-threaded path;
